...
const createCommentHelper = async (body: CreateCommentBody): Promise<CommentResponse | null> => {
  ...
  if (comment.status !== 200) {
    console.error('Error fetching created comment:', comment.statusText);
    return null;
  }
...
const updateCommentHelper = async (
  ...
  // Validate the response
  if (res.status !== 200) {
    console.error('Error updating comment:', res.statusText);
    return null;
  }
...
const CommentForm = ({
  ...
  const onSubmit = async (data: CreateCommentBody | UpdateCommentBody) => {
    ...
    // Set the loading state to prevent multiple submits
    setLoading(true);
      ...
      <fieldset className='flex flex-col gap-2' disabled={loading}>
        ...
        <div className='flex justify-end my-2'>
          <MotionTextButton
            ...
            disabled={loading}
            className='bg-muted-foreground text-muted px-3.5 py-1.5 text-sm'
          />
        </div>
      </fieldset>
    </form>
